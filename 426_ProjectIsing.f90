...
program Ising_2D_3D
	...
	open(unit=12,file="M-B(L=20,T=1.5).txt")
	! open(unit=13,file="C-T(L=10)-3D.txt")
	! open(unit=14,file="X-T(L=10)-3D.txt")
	
	!Parameters (2D):===========
	! ACTIVATE FOR 2D
	JJ=1d0 !Interaction Strength
	kB=1.0d0 !Boltzmann Constant
	mu=1.0d0 !Magnetic Moment
	L=20 !Grid Size
	T=1.5d0; dT=0.01d0; Tmax=5.0d0; !J/kB
	B=-3.0d0; dB=0.1d0; Bmax=3.0d0; !J/mu
